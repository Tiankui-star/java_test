...
public class MulArithmetic extends Arithmetic{
    public Vector<Vector<Integer>>mul_Vector=new Vector<>();
    MulArithmetic() {
        mul_Vector.add(new Vector<>());
        mul_Vector.add(new Vector<>());
    }
    ...
    {
        for(int i=0;i<mul_Vector.get(0).size();i++)
        {
            if(a==mul_Vector.get(0).get(i)||a==mul_Vector.get(1).get(i))
            {
                if(b==mul_Vector.get(0).get(i)||b==mul_Vector.get(1).get(i))
                    return true;
            }
        }
        return false;
